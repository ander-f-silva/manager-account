    ...
    try {
      HttpResponse<?> response =
          client
              .toBlocking()
              .exchange(
                  HttpRequest.POST("/accounts", new NewAccountDTO(document, fullName)),
                  NewAccountDTO.class);
    ...
    try {
      ...
      HttpResponse<?> response =
          client
              ...
              .exchange(
                  HttpRequest.PATCH(
                      "/accounts/" + accountId.toString() + "/deposit",
                      new DepositAccountDTO(amount)),
                  DepositAccountDTO.class);
      assertEquals(statusCode, response.getStatus().getCode());
    } catch (HttpClientResponseException httpClientResponseException) {
      assertEquals(statusCode, httpClientResponseException.getStatus().getCode());
    }
  ...
  void testGetAccount(ArgumentsAccessor arguments) {
    ...
    try {
      ...
      HttpResponse<?> response =
          client
              .toBlocking()
              .exchange(HttpRequest.GET("/accounts/" + accountId.toString()), AccountDTO.class);
      assertEquals(statusCode, response.getStatus().getCode());
    ...
    try {
      ...
      HttpResponse<?> response =
          client
              ...
              .exchange(
                  HttpRequest.PATCH(
                      "/accounts/" + toAccountId.toString() + "/transfer",
                      new TransferAccountDTO(fromAccountId, value)),
